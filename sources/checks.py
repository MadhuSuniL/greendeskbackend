...
import time
from Response.response import get_answer
...
def check(promt):
    ...
    #         return 'If you are trying to get the calendar for in specific year, you can send a message in the following format: <br> <center>calendar for year yyyy</center> <br>where "<b>yyyy</b>" is the year. This message will tell the program to retrieve the calendar for specific year. <br> <br> However,<br> <span style="color:red;">"I could not find the calendar because the given year is invalid.</span>'
    ...
    elif 'table' in promt and 'math' in promt or 'generate' in promt or re.search(math_table_pattern , promt, re.IGNORECASE):
    ...
    elif 'random card' == promt or 'pick a random card' in promt or ('card' in promt and 'get' in promt or 'select' in promt or 'pick' in promt or 'random' in promt) or re.search(deck_of_patterns , promt, re.IGNORECASE):
        data = {
                'id':str(time.time()),
                'key':f"Random Card",
                'img':'no-img',
        ...
        return get_answer(promt)
    
def is_math_expression(s):
    s = s.replace('=', '').replace('?', '')
    if re.search(r'\d', s) and re.search(r'[+\-*/ ]', s) and not re.search(r'[a-zA-Z]', s):
        return True
    else:
        return False
...
def GetRes(key):
    if is_math_expression(key):
        key = key.replace('=', '').replace('?', '').replace(' ','').strip()
        value = expression_patterns(key)
        data = {
            'id':str(time.time()),
            'key':f"{key} = ?",
            'img':'no-img',
